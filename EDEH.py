...
def resBlock(x, channels=16):
    ...
    w = weight_variable('conv1',[3,3,3,channels,channels])
    ...
    w = weight_variable('conv2',[3, 3, 3, channels, channels])
...
def EDEH(x, num_layers=6,feature_size=64,output_channels=1):
    ...
    w = weight_variable('conv1',[3,3,3,1,feature_size])
    ...
    w = weight_variable('conv2',[3, 3, 3, feature_size, feature_size])
    b = bias_variable('bias2',[feature_size])
    x = conv3d(x,w) + b
    x = x + conv_1

    # //////////// one more convolution after direct addition
    w = weight_variable('conv3',[3,3,3,feature_size,output_channels])
